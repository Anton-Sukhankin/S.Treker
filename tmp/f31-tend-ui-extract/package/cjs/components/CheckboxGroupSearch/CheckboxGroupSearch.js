...
const CheckboxGroupSearch = (_a) => {
    var { optionAfter, optionDescription, placeholder, allowClear, showSearch = true, virtual = false, error = false, loading = false, filterOption = true, filterOptionProp = 'value', scrollable = true, optionRender, options, onSearch, onChange, value, onScroll } = _a, props = tslib.__rest(_a, ["optionAfter", "optionDescription", "placeholder", "allowClear", "showSearch", "virtual", "error", "loading", "filterOption", "filterOptionProp", "scrollable", "optionRender", "options", "onSearch", "onChange", "value", "onScroll"]);
    const theme = tendUiTheme.useTheme();
    const [selectedValues, setSelectedValues] = tendUiHooks.UNSTABLE_useControllableStateV2({
        defaultValue: [],
        value,
        onChange,
    });
    ...
    const component = React__default["default"].useMemo(() => {
        if (virtual) {
            return (React__default["default"].createElement(Checkbox.Checkbox.Group, Object.assign({ style: createVirtualRootStyle(rowVirtualizer.getTotalSize()), fullWidth: true, layout: 'vertical' }, props, { value: selectedValues }), rowVirtualizer.getVirtualItems().map(virtualItem => {
                const filteredOption = filteredOptions[virtualItem.index];
                return (React__default["default"].createElement(Checkbox.Checkbox, { style: createVirtualItemStyle(virtualItem), key: createKey(filteredOption), value: filteredOption.value, onChange: handleChange }, filteredOption.label));
            })));
        }
        return (React__default["default"].createElement(Checkbox.Checkbox.Group, Object.assign({ fullWidth: true, layout: 'vertical' }, props, { value: selectedValues }), filteredOptions.map(filteredOption => (React__default["default"].createElement(Checkbox.Checkbox, { key: createKey(filteredOption), value: filteredOption.value, onChange: handleChange }, filteredOption.label)))));
...
exports.CheckboxGroupSearch = CheckboxGroupSearch;
