...
export const STrackerTaskCardVerticalRoute = () => {
  ...
  const renderStepContent = () => {
    if (activeStep === 0) {
      return <>
          <label className="mt-4 block text-sm font-medium" htmlFor="exact-subject">
  ...
  return <main className="min-h-full w-full bg-[#e9eef5] font-sans text-[#111827]">
      <aside className="ml-auto flex h-screen max-h-screen w-full max-w-[690px] flex-col overflow-hidden border-l border-[#d7dee8] bg-[#f4f6f9] shadow-[-12px_0_36px_rgba(15,23,42,0.16)]">
        ...
        {contentView === "default" ? <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-4">
            ...
            <section className="relative z-20 overflow-visible rounded-xl border border-[#dde3eb] bg-white shadow-sm">
              ...
              {stepsExpanded && <div id="task-steps-content" className="grid grid-cols-[190px_minmax(0,1fr)] items-start">
                <div className="self-stretch border-r border-[#e5e7eb] bg-[#f8fafc] p-3">
                  ...
                  return <li key={step} className="relative">
                        {index < steps.length - 1 && <span className={`absolute left-[19px] top-9 h-[calc(100%-12px)] w-px ${completed ? "bg-[#8bcbb3]" : "bg-[#d8dee8]"}`} aria-hidden="true" />}
                        <button type="button" disabled={!available} onClick={() => setActiveStep(index)} aria-current={active ? "step" : undefined} aria-label={`Шаг ${index + 1}: ${step}`} className={`relative flex w-full items-start gap-2.5 rounded-lg px-1.5 py-2 text-left transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#007bfb]/30 disabled:cursor-not-allowed ${active ? "bg-[#eaf5ff]" : available ? "hover:bg-white" : "opacity-55"}`}>
                          <span className={`relative z-10 grid h-7 w-7 shrink-0 place-items-center rounded-full border text-[11px] font-semibold transition ${completed ? "border-[#36b37e] bg-[#36b37e] text-white" : active ? "border-[#007bfb] bg-white text-[#006ac8] ring-2 ring-[#cce9ff]" : "border-[#cfd7e3] bg-white text-[#667085]"}`}>
                            {completed ? "✓" : index + 1}
                          </span>
                          <span className="min-w-0 pt-0.5">
                            <span className={`block text-xs font-semibold leading-4 ${active ? "text-[#005eb8]" : completed ? "text-[#287a5b]" : "text-[#475467]"}`}>{step}</span>
                            <span className="mt-0.5 block text-[10px] leading-3 text-[#667085]">{active ? "Текущий" : completed ? "Подтверждён" : available ? "Доступен" : "Недоступен"}</span>
                          </span>
                        </button>
                      </li>;
                })}
              ...
              <div className="m-4 ml-3 min-w-0 rounded-lg border border-[#dce3ec] bg-[#fafbfd] p-4">
                <div className="flex items-start justify-between gap-4">
                  <h3 className="text-base font-semibold">{steps[activeStep]}</h3>
                  <span className="text-xs text-[#667085]">
                    Обязательные поля отмечены <span className="text-[#d92d20]">*</span>
                  </span>
                </div>
                {renderStepContent()}
              </div>
