...
export const STrackerTaskCardStageCards = () => {
            ...
            <section className="relative z-20 overflow-visible rounded-xl border border-[#dde3eb] bg-white shadow-sm">
              ...
              {stepsExpanded && <div id="task-steps-content">
                <div className="border-b border-[#e5e7eb] bg-[#f8fafc] p-4">
                  ...
                  return <li key={step} className={index === steps.length - 1 ? "col-span-2" : ""}>
                        <button type="button" disabled={!available} onClick={() => setActiveStep(index)} aria-current={active ? "step" : undefined} aria-label={`Шаг ${index + 1}: ${step}`} className={`group flex min-h-[76px] w-full items-start gap-3 rounded-lg border p-3 text-left shadow-sm transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#007bfb]/30 disabled:cursor-not-allowed ${active ? "border-[#007bfb] bg-[#eef7ff] ring-1 ring-[#cce9ff]" : completed ? "border-[#b9dfcf] bg-[#f2fbf7] hover:border-[#8bcbb3]" : available ? "border-[#d8dee8] bg-white hover:border-[#b9c5d5] hover:bg-[#fbfcfe]" : "border-[#e5e7eb] bg-[#f2f4f7] opacity-70"}`}>
