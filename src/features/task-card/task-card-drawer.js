import { createButton, createDrawer, createModal, createToast } from '../../ui/components/index.js';
import { getTaskRowById } from '../../domain/task-selectors.js';
import { getTaskCardDetails } from '../../domain/task-card-details.js';
import { createTaskCardState } from '../../domain/task-card-state.js';
import { createLegoDocumentForm } from './lego-document-form.js';
...
function createTaskCardDrawer({ task, state } = {}) {
  ...
  const drawer = createDrawer({
    ...
    content,
    footer,
    closeLabel: 'Закрыть карточку задачи',
  });
  drawer.element.setAttribute('aria-modal', 'false');
  ...
  function showTakeover(view, trigger) {
    ...
    const back = createButton({
      ...
      ariaLabel: 'Вернуться к карточке задачи',
      icon: icons.back(),
      variant: 'text',
      className: 'stand-task-takeover__back',
      onClick: () => {
        takeover.hidden = true;
  ...
  return {
    ...
    open(trigger) {
      ...
      form.refresh();
      updateFooter();
      // Commit the off-screen position before activation so the first open also animates.
      drawer.element.getBoundingClientRect();
      drawer.open(trigger);
    },
    close() {
      if (closeConfirmation.element.open) closeConfirmation.close();
      drawer.close();
    },
    requestClose,
    destroy() {
...
export function initTaskCardDrawer() {
  const states = new Map();
  let activeTaskId = null;
  let activeDrawer = null;

  const open = (taskId, trigger) => {
    const taskRow = getTaskRowById(taskId);
    const task = getTaskCardDetails(taskRow);
    if (!task) return;

    if (activeTaskId !== taskId) {
      activeDrawer?.destroy();
      const state = states.get(taskId) || createTaskCardState(task);
      states.set(taskId, state);
      activeDrawer = createTaskCardDrawer({ task, state });
      activeDrawer.mount();
      activeTaskId = taskId;
    }
    activeDrawer.open(trigger);
  };

  const onTaskTitleClick = event => {
    const trigger = event.target.closest('.js-open-task-card');
    if (!trigger) return;
    event.preventDefault();
    event.stopPropagation();
    open(trigger.dataset.taskId, trigger);
  };
  document.addEventListener('click', onTaskTitleClick);

  return {
    open,
    close() { activeDrawer?.close(); },
    requestClose() { activeDrawer?.requestClose(); },
    destroy() {
      document.removeEventListener('click', onTaskTitleClick);
      activeDrawer?.destroy();
      activeDrawer = null;
      activeTaskId = null;
    },
